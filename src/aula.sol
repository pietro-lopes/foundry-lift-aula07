...
contract LiftAMM is ERC20 {
    ...
    function getDeadline(uint256 _minutes) external view returns (uint256) {
        ...
        returns (uint256 liquidity)
    {
        ...
        (uint256 balanceA, uint256 balanceB) = AmmLibrary.getReserves(
            tokenA,
            tokenB
        );
        ...
        returns (uint256 amountA, uint256 amountB)
    {
        ...
        uint256 denominator = totalSupply() / liquidity;
    ...
    function swap(
        ...
        require(
            balanceA > 0 && balanceB > 0 && amountIn < balanceA,
            "Not enought liquidity tokenA"
        );

        amountOut = AmmLibrary.getAmountOut(amountIn, balanceA, balanceB, fee);
        require(
            amountOut > 0 && amountOut < balanceB,
            "Not enought liquidity tokenB"
        );
        require(
